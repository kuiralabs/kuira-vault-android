...
// The Private Vault content, rendered inside MainActivity's tab host. The floating wallet/sigil
// pills are hoisted to the window root (MainActivity). The action progress (deploy / deposit /
// propose / approve / execute) is PINNED at the bottom here — outside the scroll — so it stays
// visible wherever the user has scrolled and whichever button they tapped.
@Composable
fun PrivateVaultScreen(
    ...
    Box(modifier = modifier.fillMaxSize()) {
        Column(
            ...
            PrivateVaultCard(viewModel = viewModel)
        }
